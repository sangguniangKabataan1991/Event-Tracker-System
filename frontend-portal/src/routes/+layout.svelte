  ...
  <div class="min-h-screen bg-slate-50">
    ...
    >
      <div class="flex w-full items-center px-4 py-3">
        ...
        {#if $user}
          <!-- Profile avatar + dropdown ONLY (no standalone My Applications link) -->
          <div class="relative" bind:this={profileMenuRef}>
            ...
            >
              <div class="relative w-7 h-7 rounded-full overflow-hidden shrink-0">
                {#if ($user as any)?.avatar_url}
                  <img src={($user as any).avatar_url} alt="Profile" class="w-full h-full object-cover" />
                {:else}
            ...
            {#if showProfileMenu}
              <div class="absolute right-0 top-full mt-2 z-50 w-60 rounded-2xl overflow-hidden shadow-2xl border"
                ...
                <div class="px-4 py-4" style="border-bottom:1px solid rgba(255,255,255,0.08);">
                  <div class="flex items-center gap-3 mb-3">
                    <div class="relative group shrink-0">
                      ...
                      >
                        {#if uploadingAvatar}
                          <div class="w-4 h-4 border-2 border-white/30 rounded-full"
                              style="border-top-color:white; animation:spin 0.8s linear infinite;"></div>
                        {:else}
                    ...
                    <div class="min-w-0">
                      <div class="text-sm font-semibold text-white truncate">{($user as any)?.full_name}</div>
                      <div class="text-xs truncate" style="color:rgba(255,255,255,0.45);">@{($user as any)?.username}</div>
                      <button
                        onclick={() => fileInputRef?.click()}
                        disabled={uploadingAvatar}
                  ...
                  {#if avatarError}
                    <div class="text-xs rounded-lg px-3 py-2 mt-1"
                ...
                <input type="file" accept="image/*" class="hidden"
                      bind:this={fileInputRef} onchange={handleAvatarChange} />
